...
#declare UseCaseDiagram_77u48e854451c4ee75ae849d72474c5eb8f = union {
  ...
  cylinder {-z, z 1 texture { T_Wood10 } scale <3.6, 0.8, 0.05> texture { pigment { object { union { 
    text { ttf LabelFont "これはユースケースです", 1, 0 translate <-1, -0, 0> }
    text { ttf LabelFont "アクターが「～をする」と描いてね", 1, 0 translate <-1, -1, 0> }
     scale <0.5, 0.5, 1> translate <-0.5, 1, -0.5> } color<1,1,1,1> color<0,0,0,1> }}
    finish { phong 1 ambient 0.2 } scale <.7, .5, 10> translate <-1.0, -0.5, -2>
  } scale 64  translate <326.67, -193.99, 0.00>}
  cylinder {-z, z 1 texture { T_Wood10 } scale <2.6, 0.6000000000000001, 0.05> texture { pigment { object { union { 
    text { ttf LabelFont "これはユースケースです", 1, 0 translate <-1, -0, 0> }
     scale <0.5, 0.5, 1> translate <-0.5, 1, -0.5> } color<1,1,1,1> color<0,0,0,1> }}
    finish { phong 1 ambient 0.2 } scale <.7, .5, 10> translate <-0.6875, -0.5, -2>
  } scale 64  translate <311.00, -308.17, 0.00>}
}
object { UseCaseDiagram_77u48e854451c4ee75ae849d72474c5eb8f }
#declare EYE = <268.83, -445.17, -426.00>;
#declare FOCUS = <268.83, -205.17, 0.00>;

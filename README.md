...
サブダイアグラムの手動設定
1. POVRayスクリプトファイルで、サブダイアグラムオブジェクトのコメントを外す。
2. サブダイアグラムオブジェクトを宣言しているPOVRayスクリプトファイルを探し、宣言部(#declare ダイアグラムオブジェクト名 = ...)をコピーする。
3. ダイアグラムオブジェクトの宣言部の前に、ペーストする。
4. 三層以上階層や複数階層であるときは、2,3を繰り返す。
5. POVRayツールで。POVRayスクリプトファイル(.pov)を開いて、レンダリングする。
